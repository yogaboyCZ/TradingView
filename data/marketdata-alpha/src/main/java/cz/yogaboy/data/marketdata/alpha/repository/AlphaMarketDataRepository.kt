...
class AlphaMarketDataRepository(
    ...
    override suspend fun getLatestPrice(ticker: String): Price? {
        //TODO add handling for errors and logging coz alpha is a bit unreliable
        val env = api.getGlobalQuote(symbol = ticker, apiKey = apiKey)
        val result  = env.toDomain(fallbackTicker = ticker)
        return result
    }
}
